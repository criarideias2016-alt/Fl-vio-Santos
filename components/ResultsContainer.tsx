...
import type { AnalysisResult, CompetitiveAnalysis, DetailedScorecardResult, GroundingChunk, IdeasResult, KeywordsResult, LocalRankingResult, RadiusAnalysisResult, ResponsesResult, SeoActionsResult, OptimizationBenefits, HeadToHeadAnalysis, CustomerProfile, SentimentAnalysis } from '../types';
import { ResultsDisplay } from './ResultsDisplay';
import { CompetitiveAnalysisDisplay } from './CompetitiveAnalysisDisplay';
import { KeywordDisplay, KeywordVolumeAnalyzer } from './KeywordDisplay';
...
import { OptimizationBenefitsDisplay } from './OptimizationBenefitsDisplay';
import { HeadToHeadAnalysisDisplay } from './HeadToHeadAnalysisDisplay';
...
type SectionId = 'summary' | 'scorecard' | 'customer-profile' | 'sentiment' | 'benefits' | 'head-to-head' | 'suggestions' | 'keywords' | 'volume' | 'responses' | 'seo' | 'radius' | 'ideas';

const navItems: { id: SectionId, label: string, icon: React.ReactNode, color: string }[] = [
...
export const ResultsContainer: React.FC<ResultsContainerProps> = ({
  ...
  const renderSection = (id: SectionId, component: React.ReactNode) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-slate-200/80">
        <div className={isGeneratingPdf ? '' : 'md:grid md:grid-cols-12'}>
            <aside className={isGeneratingPdf ? 'hidden' : 'md:col-span-3 lg:col-span-2 p-4 border-r border-slate-200'}>
                <div className="flex justify-between items-center mb-4">
                    <h3 className="font-bold text-slate-800">Relatório</h3>
                    <button 
            ...
            <main id="report-content" className={isGeneratingPdf ? 'p-6' : 'md:col-span-9 lg:col-span-10 p-6 lg:p-8'}>
