...
import type { ResponsesResult, ResponseTemplate } from '../types.ts';
...
const TemplateCard: FC<{ template: ResponseTemplate }> = ({ template }) => {
    ...
    return (
        <div className="bg-white dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700 rounded-lg p-5">
            <div className="flex justify-between items-start">
                <h4 className="text-md font-bold text-gray-700 dark:text-slate-300 mb-2">{template.title}</h4>
                <button onClick={handleCopy} title="Copiar texto" className="text-gray-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors flex-shrink-0">
                    {isCopied ? (
            ...
            <p className="text-gray-600 dark:text-slate-400 whitespace-pre-line">{template.text}</p>
        </div>
    );
};

export const ResponseTemplatesDisplay: FC<ResponseTemplatesDisplayProps> = ({ result }) => {
  if (!result || !Array.isArray(result.positive) || !Array.isArray(result.negative)) {
    return (
      <div className="p-6 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
        <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200 border-b dark:border-slate-700 pb-3 mb-6">Modelos de Resposta para Avaliações</h2>
        <p className="text-slate-500 dark:text-slate-400">Não foi possível carregar os modelos de resposta. Tente novamente mais tarde.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200 border-b dark:border-slate-700 pb-3 mb-6">Modelos de Resposta para Avaliações</h2>
        <p className="text-gray-600 dark:text-slate-400 mb-6">Use estes modelos como ponto de partida para interagir com seus clientes e melhorar sua reputação online.</p>
