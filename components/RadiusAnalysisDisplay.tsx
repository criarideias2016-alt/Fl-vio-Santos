...
import type { RadiusAnalysisResult } from '../types.ts';

interface RadiusAnalysisDisplayProps {
  result: RadiusAnalysisResult;
}

const NeighborhoodBar: FC<{ name: string; score: number }> = ({ name, score }) => {
    return (
        <div className="flex items-center gap-4 text-sm">
            <div className="w-1/3 text-slate-700 dark:text-slate-300 truncate text-right pr-2 flex items-center justify-end gap-2" title={name}>
                <span>{name}</span>
            ...
            <div className="w-2/3 bg-slate-200 dark:bg-slate-700 rounded-md h-8">
                <div 
                ...
                >
                    <span className={`font-bold text-xs ${score > 40 ? 'text-white' : 'text-blue-800 dark:text-blue-200'}`}>{score}%</span>
                </div>
            </div>
        </div>
    );
...
export const RadiusAnalysisDisplay: FC<RadiusAnalysisDisplayProps> = ({ result }) => {
    if (!result || !Array.isArray(result.neighborhoods)) {
      return (
        <div className="p-6 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200 border-b dark:border-slate-700 pb-3 mb-6">Análise de Interesse por Bairro</h2>
          <p className="text-slate-500 dark:text-slate-400">Não foi possível carregar a análise de raio. Tente novamente mais tarde.</p>
        </div>
      );
    }
    ...
    return (
        <div className="space-y-8">
            <div>
                <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200 border-b dark:border-slate-700 pb-3 mb-6">Análise de Interesse por Bairro</h2>
                <p className="text-slate-600 dark:text-slate-400 mb-6">Gráfico dos bairros com maior volume de busca para o seu segmento de negócio, em um raio de até 10km.</p>
                
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
                    <div className="lg:col-span-3 p-6 bg-white dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="lg:col-span-2 p-6 bg-blue-50/60 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800/50 h-full">
                        ...
                        <p className="text-blue-900/90 dark:text-blue-200/90 whitespace-pre-line">{result.analysisSummary}</p>
                    </div>
                </div>
            </div>
        </div>
