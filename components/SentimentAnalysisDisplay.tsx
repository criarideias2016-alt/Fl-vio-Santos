...
import type { SentimentAnalysis, SentimentTheme } from '../types.ts';

interface SentimentAnalysisDisplayProps {
  result: SentimentAnalysis;
}

const ThemeCard: FC<{ theme: SentimentTheme; type: 'positive' | 'negative' }> = ({ theme, type }) => {
    const styles = {
        positive: {
        ...
        <div className={`p-5 rounded-lg border ${style.bgColor} ${style.borderColor}`}>
            <div className="flex items-start gap-4">
                ...
                <div>
                    ...
                    <p className="text-slate-600 dark:text-slate-400 mt-1">{theme.summary}</p>
...
export const SentimentAnalysisDisplay: FC<SentimentAnalysisDisplayProps> = ({ result }) => {
  if (!result || !Array.isArray(result.positiveThemes) || !Array.isArray(result.negativeThemes)) {
    return (
      <div className="p-6 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
        <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200 border-b dark:border-slate-700 pb-3 mb-6">Análise de Sentimento das Avaliações</h2>
        <p className="text-slate-500 dark:text-slate-400">Não foi possível carregar a análise de sentimento. Tente novamente mais tarde.</p>
      </div>
    );
  }
