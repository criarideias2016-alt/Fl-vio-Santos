...
import type { IdeasResult, Idea } from '../types.ts';
...
const IdeaCard: FC<{ idea: Idea }> = ({ idea }) => {
    ...
    return (
        <div className="bg-white dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700 rounded-lg p-5">
            <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                    <span className="text-sm font-medium text-gray-500 dark:text-slate-400">{idea.category}</span>
                    <h4 className="text-md font-bold text-gray-700 dark:text-slate-300">{idea.title}</h4>
                </div>
            </div>
            <p className="text-gray-600 dark:text-slate-400 text-sm">{idea.description}</p>
        </div>
    );
};

export const IdeaGeneratorDisplay: FC<IdeaGeneratorDisplayProps> = ({ result }) => {
  if (!result || !Array.isArray(result.ideas) || !result.photo360) {
    return (
      <div className="p-6 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
        <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200 border-b dark:border-slate-700 pb-3 mb-6">Gerador de Ideias Criativas</h2>
        <p className="text-slate-500 dark:text-slate-400">Não foi possível carregar as ideias. Tente novamente mais tarde.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200 border-b dark:border-slate-700 pb-3 mb-6">Gerador de Ideias Criativas</h2>
        <p className="text-gray-600 dark:text-slate-400 mb-6">Inspire-se com estas sugestões para engajar sua comunidade local e atrair novos clientes.</p>
        
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-gray-700 dark:text-slate-300">Ideias de Marketing e Conteúdo</h3>
            {result.ideas.map((idea, index) => (
                <IdeaCard key={index} idea={idea} />
            ))}
          </div>

          <div className="p-6 bg-gradient-to-br from-blue-100 to-gray-50 dark:from-blue-900/30 dark:to-slate-800/30 rounded-lg border-2 border-blue-300 dark:border-blue-700/50 shadow-lg shadow-blue-500/10">
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-xl font-bold text-gray-800 dark:text-slate-200">{result.photo360.title}</h3>
                </div>
                <p className="text-blue-900/90 dark:text-blue-200/90 whitespace-pre-line">{result.photo360.description}</p>
          </div>
        </div>
      </div>
    </div>
